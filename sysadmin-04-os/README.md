...
6. Запустите любой долгоживущий процесс (не `ls`, который отработает мгновенно, а, например, `sleep 1h`) в отдельном неймспейсе процессов; покажите, что ваш процесс работает под PID 1 через `nsenter`. Для простоты работайте в данном задании под root (`sudo -i`). Под обычным пользователем требуются дополнительные опции (`--map-root-user`) и т.д.
			...
			root@vagrant:~# ps
				PID TTY          TIME CMD
			  33133 pts/0    00:00:00 sudo
			  33134 pts/0    00:00:00 bash
			  33456 pts/0    00:00:00 unshare
			  33457 pts/0    00:00:00 sleep
			  33486 pts/0    00:00:00 ps
			  
			root@vagrant:~# nsenter --target 33457 --pid --mount
			
			root@vagrant:/# ps aux
			USER         PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND
			root           1  0.0  0.0   8076   592 pts/0    S    10:22   0:00 sleep 1h
			root           2  0.0  0.3   9836  3976 pts/0    S    10:30   0:00 -bash
			root          11  0.0  0.3  11492  3260 pts/0    R+   10:30   0:00 ps aux
